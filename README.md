...
**Crash "EMFILE: too many open files" + szybsze pobieranie.** Biblioteka do pobierania assetów nie miała żadnego limitu jednoczesnych pobrań, co na Windows przy tysiącach małych plików potrafiło wykrztusić system i zawiesić całą aplikację tak mocno, że nie dało się jej zamknąć nawet Menadżerem Zadań. Naprawione dwiema rzeczami:
1. **Limit 16 jednoczesnych pobrań** zamiast bez limitu — stabilne i, wbrew pozorom, **szybsze** niż wcześniej (bo wcześniej się dławiło/zawieszało zamiast realnie pobierać).
2. **Globalne przechwytywanie nieobsłużonych błędów** — jeśli coś mimo wszystko pójdzie nie tak, błąd trafia teraz do okna logów (📄) zamiast pokazywać natywne okno "A JavaScript error occurred", które potrafiło zablokować cały proces na dobre.
...
```bash
wget https://github.com/monykPL/MelonClientSource/releases/latest/download/MelonClient.AppImage
chmod +x MelonClient.AppImage
./MelonClient.AppImage
```
...
```bash
curl -sL https://raw.githubusercontent.com/monykPL/MelonClientSource/main/scripts/install-linux.sh | bash
```
...
**Adres repo (`monykPL/MelonClientSource`) jest już wpisany na sztywno** w skrypcie i w komendach powyżej. Zostają Ci tylko:
1. Wrzuć aktualny kod na to repo na GitHubie.
2. Zbuduj `.AppImage` (`npm run dist:linux` na maszynie z Linuksem) i wrzuć go jako załącznik do **GitHub Release** (Releases → Create a new release → dołącz plik). Dzięki stałej nazwie pliku (`MelonClient.AppImage`) link do pobrania nie zmieni się przy kolejnych wersjach — wystarczy podmieniać załącznik w nowych release'ach.

Bez opublikowanego Release'a z załączonym plikiem obie komendy powyżej nie będą miały czego pobrać.
